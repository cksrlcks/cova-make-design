...
- **메인 페이지 히어로는 스크린 풀사이즈(`min-height: 92~100vh`) + 오버사이즈 타이포를 기본**으로 한다. 타이포 자체를 비주얼로 쓰는 해외 어워드·국내 에이전시 문법을 적극 참고한다([design-trends.md](design-trends.md) 무브 1·10·11). 서브·관리자 유형에는 강제하지 않는다.
