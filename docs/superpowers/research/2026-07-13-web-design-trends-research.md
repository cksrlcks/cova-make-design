...
- 무드: 애플 iOS 26 "Liquid Glass" 이후 전 산업으로 번진, 빛을 굴절시키는 투명 재질의 미학.
  ...
  아니라 밝고 적응형인 무드. **D9(뉴트로 인터페이스)와도 다르다:** D9의 광택 블루-그린(에어로)·
  겔 버튼이 유리/겔 질감을 공유하지만, D9는 Y2K·홀로그램·렌즈플레어·윈도우 크롬 인용의 레트로
  향수를 파는 반면, 이 방향은 그런 레트로·픽셀·OS 크롬 요소 없이 현대적 중립 팔레트 위에서
  feDisplacementMap 굴절 자체를 순수하게 보여준다.
...
- 주의: `backdrop-filter`+SVG `feDisplacementMap` 조합은 **비-Chromium 브라우저(Safari·Firefox,
  최신 버전 포함)에서 깨진다** — 구형이 아니라 현행 엔진의 상시 갭이다(LogRocket·kube.io 확인).
  국내 모바일 트래픽 상당수가 iOS Safari이므로 폴백(반투명 단색/저채도 글래스 배경으로 열화)은
  선택적 열화가 아니라 **필수 기본 경로**다 — Task 3는 무브 레시피에서 이 폴백을 축소하지 말 것.
  배경이 밋밋하면 굴절 효과 자체가 안 보이므로 반드시 그러데이션·사진 위에서만 사용.
