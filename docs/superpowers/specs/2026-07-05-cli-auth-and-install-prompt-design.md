...
1. `POST /sessions` — body 없음. 세션 생성 →
   `{ authId, verifyUrl: "<origin>/cli-auth/<authId>", expiresIn: 600 }`
...
### 승인 페이지 `/cli-auth/[authId]` (스튜디오 레이아웃 밖 독립 라우트)

- `/studio/*`가 아닌 이유: studio 레이아웃이 비로그인 시 `returnTo=/studio`로 고정
  리다이렉트해서 로그인 후 승인 페이지로 복귀하지 못한다. 독립 라우트에서 자체 가드로
  `redirect("/login?returnTo=/cli-auth/<authId>")` 처리 (OAuth 동의 화면처럼 단독 페이지)
